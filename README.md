...
2. **Compaction-filter exposure — nuanced (corrected, §18.6).** For *TTL* cleanup this is **NOT** a gap: SlateDB has a **built-in** TTL compaction filter that runs automatically, and `putWithOptions(PutOptions{ttl})` / `mergeWithOptions` **ARE bound** in Java (verified `javap`). So processing-time TTL expiry works from Java today — same role Flink's `FlinkCompactionFilter` plays for RocksDB State TTL. The gap is only *arbitrary custom* compaction filters (RFC 0017): not bound, and in fact **compiled off** in the shipped JAR (feature not in the binding's `all` set), and enabling them carries a snapshot-consistency caveat (§18.6). Also: SlateDB TTL is **wall-clock**, so it maps to processing-time `OnCreateAndWrite` TTL, **not** event-time or `OnReadAndWrite` refresh (the `clock` is a builder Component and is **not** injectable in the binding — §18.5).
...
### 18.6 TTL cleanup works from Java today (RFC 0003/0017) — **[VERIFIED]**, corrects §15.3
The doc implied background TTL cleanup is blocked because "the compaction filter isn't in the binding." That conflates two things (see corrected §15.3): SlateDB has a **built-in TTL compaction filter** that runs automatically (no API needed), and **`putWithOptions(PutOptions{ttl})` / `mergeWithOptions` ARE bound** (verified `javap`). So processing-time TTL expiry — the exact role Flink's `FlinkCompactionFilter` plays for RocksDB State TTL — works from Java now. Only *arbitrary custom* filters (RFC 0017) are missing: not bound, **compiled off** in the shipped JAR, and enabling them risks snapshot-consistency (a hazard for §6.6 clone-restore exactly-once). Limit: SlateDB TTL is **wall-clock**, so processing-time `OnCreateAndWrite` only — not event-time / `OnReadAndWrite` refresh. The logical `clock` is a builder **Component NOT injectable in the binding** (no `with_clock`), so `create_ts`/TTL can't be tied to Flink watermarks (compounds §13.7).

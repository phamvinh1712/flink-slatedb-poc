...
The WAL provides **durability** — it makes a write crash-safe before the larger memtable→L0 flush. Log-ordered in-memory buffer → flushed as SST-format files to object storage → replayed from a manifest watermark on recovery, de-duped by sequence number. Verified against `wal_buffer.rs`, `batch_write.rs`, `db.rs`, `schemas/manifest.fbs`.
...
- **Keep the WAL ON only if** you also read/serve from SlateDB *between* checkpoints and can't tolerate re-processing on recovery, or you use CDC/`WalReader` (§18.10). If you keep it on, consider `withWalObjectStore` → a low-latency store (§18.8) to cut the write-latency cost.
...
- **Consequence:** the object-store choice is load-bearing for *correctness*, not just performance. **AWS S3 only added conditional PUT (`If-None-Match`) in Aug 2024**; older/on-prem MinIO, various S3 gateways, or a misconfigured bucket may lack it → the fencing/exactly-once mechanism silently has no substrate.
- **Blind spot:** every PoC ran on `file:///`/`memory:///`, which provide atomic/CAS semantics for free — so the single most safety-critical external dependency was **never exercised**. Before adopting: confirm your target store supports conditional writes (or wire the txn-store fallback, if the binding exposes it — unverified). Amends §2.1 (store list), §4, §6.6.
...
- **Expiry → unrestorable.** A SlateDB checkpoint has `checkpoint_expire_time_s`; if it expires (or is GC'd) before Flink restores from a *retained* checkpoint/savepoint (Flink may hold these for days), the clone source is gone → **silent state loss on restore**. Use `lifetime=None` or `refreshCheckpoint` to guarantee it outlives the referencing Flink checkpoint.
...
⚠️ **Gotcha:** the default recorder is **no-op → zero metrics unless you wire one** (`withMetricsRecorder`). And whether a Java operator reads snapshots directly vs. implements the `MetricsRecorder` callback interface (FFI hot-path cost) is a real binding-usability question (RFC 0021 flags UniFFI exposure as follow-up). New §-worthy area; amends §9.4.
...
- **Subcompactions (RFC 0028)** [RFC, likely post-0.14.1] — a forthcoming `max_subcompactions` (default 4) multiplies intra-compaction parallelism (`max_concurrent_compactions × max_subcompactions` ≈ 16 CPU threads/DB) — tune down for embedded use; another argument for the sidecar (§7).

...
No Docker; just plain `mvn` + local JDKs. **The full design doc is included below** (see "Design Doc" section);
...
```
(`exec:java` does NOT work for the MiniCluster job due to a classloader issue; use plain `java` as above.)
...
```
(No `--enable-native-access` / `--add-opens`; those are FFM-only, and JNA needs neither.)
...
- `SlateDbRescaleE2E`: the **§6.4 rescale merge** (clone-with-projection + scan-copy) on the handle list
  ...
  (no `--add-opens` needed, since it uses no Flink runtime, only `KeyGroupRangeAssignment` + SlateDB).
- `FlinkHybridTieringE2E`: the **§13 hybrid**, a real Flink `KeyedProcessFunction` with hot state in Flink
...
1. **Changelog State Backend** (`state.backend.changelog`): exists in **both**, fully integrated, exactly-once. If the goal is faster checkpoints/recovery/rescale (not literally removing local disk), it's the **cheapest win, so try first.**
...
SlateDB is an LSM key-value store whose **durable state lives entirely in object storage**; the process holds only volatile state (memtables + caches). This diagram is verified against the v0.14.1 source; the §§ below expand each piece.
...
**Reading it:**
- **Write = two steps:** WAL buffer first for durability (→ `wal/*.sst`), then the memtable (`batch_write.rs`). A full memtable freezes, and the flusher writes it as an **L0 SST**.
...
- **Rescaling** reassigns key-group ranges to subtasks. A new subtask's range may span parts of several old instances, which is the core rescale problem.
...
- **Shard-per-bucket with N tuned close to typical P** (N/P in low single digits) gives the **best of both:** zero-copy rescale *and* low per-subtask overhead. This is the sweet spot. Do **not** set N enormous "to be safe."
- **Shard-per-bucket with N ≫ P:** the rescale win is real, but per-subtask overhead (N/P × unflushed + N/P compactors) is significant. Only worth it if rescales are frequent enough to justify it, *and* you apply all mitigations below.
- **One-DB-per-subtask** gives the **lowest steady-state overhead (exactly 1 instance/subtask)** and pays at rescale via scan-copy. **If rescales are rare and N/P would be large, this is the better choice**, the opposite of what an earlier draft implied.
...
- UniFFI-generated (via `uniffi-bindgen-java`); **generated `.java` sources are NOT committed** and are produced into `build/generated/...` at build time. Native lib bundled as a JNA resource.
...
| `shutdown()` | `CompletableFuture<Void>` | **the graceful close** (`close` is reserved for the AutoCloseable destructor) |
| `status()` | `DbStatus` | **synchronous** (only non-async method) |

### 5.3 Checkpoint & clone (NOT on `Db` — on `Admin`)

- **Checkpoint:** `admin.createDetachedCheckpoint(CheckpointOptions)` → `CompletableFuture<CheckpointCreateResult>`. "Detached" means it outlives the `Db` handle (exactly what we need to fold into a Flink checkpoint). Also on `Admin`: `listCheckpoints`, `deleteCheckpoint`, `refreshCheckpoint`, `runGcOnce`.
- **Clone** (shallow, shares SSTs):
  ...
  ```
  Clone is a **shallow copy**: new manifest references the same SSTs, no copy except WAL. **But the clone reads the parent's SSTs at the parent path and pins them alive via the checkpoint**, so GC must be reference-aware; repeated clones build a chain (see §6.5).

### 5.4 Builders — ⚠️ NOT fluent
Exported builder setters return `Result<()>`, i.e. **`void` in Java** (throw on error), *not* `this`. Call each as a separate statement.
...
| Aspect | Value | Note |
|---|---|---|
| Default | **ON** | An SST gets a filter when it holds **≥ `min_filter_keys` = 1000** keys (`config.rs` default). Smaller SSTs skip it, since a full scan is cheaper than a filter lookup there. |
...
Same binary format for both; `sst_type` (Wal vs Compacted) and the manifest tell them apart. An SST is sorted **data blocks** + a **footer**:
...
The LSM tree lives **here**, not in the SSTs. Framing is `[u16 big-endian version][flatbuffer ManifestV2]`. A new manifest is written on every state change (flush/compaction/checkpoint) via **CAS**; the highest id is the current state.
...
Kept separate from the manifest so tracking in-flight jobs doesn't churn it. Framing is `[u16 BE version][flatbuffer CompactionsV1]`.
...
- Millions of SSTs make a large manifest that must be serialized + CAS-written on each change and re-read on recovery. Bounded by memory + manifest write latency, not a hard limit.
...
### 5E. WAL design & recovery

The WAL provides **durability**: it makes a write crash-safe before the larger memtable→L0 flush. A log-ordered in-memory buffer is flushed as SST-format files to object storage, then replayed from a manifest watermark on recovery, de-duped by sequence number. Verified against `wal_buffer.rs`, `batch_write.rs`, `db.rs`, `schemas/manifest.fbs`.

```
  WAL = DURABILITY log. Decoupled from the memtable→L0 flush: they advance independently
  (this decoupling is the §16.2 flush trap). WAL reuses the SST format on disk.

┌──────────────────────────────────────────────────────────────────────────────────────┐
│  WRITE:  put/write(batch)  applied ATOMICALLY to BOTH ─────────────                  │
│    • WAL buffer  (log-ordered by seq — the durability record)                        │
│    • memtable    (key-sorted — the queryable index)                                  │
└──────────────────────────────────────────────────────────────────────────────────────┘

┌──────────────────────────────────────────────────────────────────────────────────────┐
│  IN MEMORY: WalBufferManager                                                         │
│    current_wal  ──freeze──►  [imm wal][imm wal]…   (queue pending flush)             │
│    flush triggers:  max_wal_size  |  flush_interval=100ms  |  explicit flush()       │
├──────────────────────────────────────────────────────────────────────────────────────┤
│  ON OBJECT STORE:  wal/00000000000000000042.sst   (SST format, sst_type=Wal,         │
│                     first_entry = a SEQ NUMBER not a key; monotonic ids)             │
│     → can live on a SEPARATE fast store (withWalObjectStore, §18.8: S3 Express)      │
└──────────────────────────────────────────────────────────────────────────────────────┘
        │ periodic 100ms flush = the group-commit await_durable=true waits on (~1ms/write)
        ▼
   ═══ WAL and memtable→L0 are DECOUPLED ═══
   • WAL flushes often (durability);  memtable flushes to L0 only when full (64MiB)
   • on L0 flush, manifest records replay_after_wal_id = WAL id now covered by that L0
   • ⚡ so WAL-durable ≠ manifest-captured → a CHECKPOINT needs flushWithOptions(MemTable) (§16.2)

┌──────────────────────────────────────────────────────────────────────────────────────┐
│  RECOVERY (replay_wal): rebuild memtable from UNFLUSHED WAL only                     │
│    replay range = [ replay_after_wal_id + 1 .. newest wal id ]   (bounded, not all)  │
│    skip entries with seq < last_l0_seq   (already in L0 → idempotent replay)         │
│    read WAL SSTs ascending → refill memtables → re-freeze at l0_sst_size             │
│    missing WAL SST mid-replay + writer_epoch bumped → Fenced (§18.9 zombie writer)   │
└──────────────────────────────────────────────────────────────────────────────────────┘

  Optional: wal_enabled=false → writes skip the WAL, durable only at L0 flush. Faster;
  loses sub-flush durability. Valid for Flink (barrier memtable-flush is the true point).
```

**Should you use the WAL with Flink?** Usually **NO; `wal_enabled=false` is the better default** for the exactly-once design. Reasoning:
- Flink's exactly-once doesn't need SlateDB's *per-write* durability. The true durability point is the **memtable flush at the checkpoint barrier** (§6.3/§16.2): `snapshotState` does `flushWithOptions(MemTable)` + `createDetachedCheckpoint`, making all pre-barrier state durable before the Flink checkpoint acks. Writes lost in a crash *before* that barrier are **re-generated by Flink source replay**, which is what makes the whole pipeline exactly-once (§18.7).
- So the WAL's job (protecting sub-checkpoint writes) is redundant with Flink's own replay, and it costs the ~1ms/write group-commit latency (§16.14) plus extra object-store writes/cost. Turning it off is **faster and cheaper** with no exactly-once loss.
- **Keep the WAL ON only if** you also read/serve from SlateDB *between* checkpoints and can't tolerate re-processing on recovery, or you use CDC/`WalReader` (§18.10). If you keep it on, consider `withWalObjectStore` pointing at a low-latency store (§18.8) to cut the write-latency cost.
- ⚠️ Either way, the **checkpoint still needs `flushWithOptions(MemTable)`** (§16.2). That's a *memtable* flush and is unrelated to `wal_enabled`. Don't confuse "WAL off" with "no flush at the barrier."
...
[ keyGroup (1–2 bytes) ][ stateId ][ serialized key ][ namespace ]
   ^ leading, enables       ^ which ValueState/MapState/…
     ...
     scans
```
`keyGroupPrefixBytes` = 1 if `maxParallelism <= 128`, else 2 (mirror Flink). Sorted LSM order means a *range* of key groups is one contiguous byte range, the same trick Flink plays for rescale.
...
The one boundary that's yours is **`CompletableFuture` vs. Flink's single-threaded-per-subtask operator model.** Two shapes:
...
```
RMW here is correct because Flink processes one element at a time per subtask. The cost is throughput. Mitigate with an in-memory write-back cache for the hot set, `WriteBatch` coalescing, and flush-at-barrier.
...
**`RichAsyncFunction`** keeps N requests in flight for throughput, **but two in-flight updates to the same key can interleave, and there are no timers / no Flink keyed state inside it.**
...
The manual scan-copy version below predates the projection/union path (§16.17); it still works and is what `SlateDbRescaleE2E` used, but **the projection/union approach above (§16.18) is cleaner, with no byte copy.** Emulates `RocksDBIncrementalRestoreOperation.restoreWithRescaling`:
```java
...
- **Physical reclaim** has options: `withProjectionRange` on the clone to only bring your slice in the first place; or background scan-and-tombstone (point deletes); or let it ride and GC on next full rebuild.

Key property: each key group is owned by exactly one old subtask, giving **no key-level merge conflicts, no last-writer-wins**, a clean partition copy.
...
- Old pre-rescale DBs **cannot be blindly deleted** while any clone references their SSTs, so **GC must be checkpoint-reference-aware**.
- Repeated rescales build **chains of clones**, so run periodic full compaction/rewrite to collapse the chain, else read amplification and cross-path fan-out grow.
...
**Consequence, the restore procedure is mandatory, not optional:** on `initializeState` you **must NOT** plain-reopen the writer on the existing DB (that replays post-barrier WAL). Instead **clone from the pinned checkpoint** (`CloneSourceSpec.withCheckpoint`), giving a fresh writable DB rooted at exactly the checkpoint's manifest, **WAL not carried over**, then write to the clone. Clone-on-restore is therefore not just a rescale tool (§6.4); **it is the mechanism that makes exactly-once possible at all.** A direct writer reopen is unsafe.

Alternative: **idempotent/`merge`-only writes** that are replay-safe, so resurrection doesn't matter.

> ✅ **VERIFIED SAFE (§12.8):** `clone.rs` reads the parent manifest at `checkpoint.manifest_id` and `copy_wal_ssts` bounds WAL copy to the checkpoint's `[replay_after_wal_id+1, next_wal_sst_id)`, so post-checkpoint WAL is **not carried over**. Test `should_clone_from_checkpoint` confirms. **So clone-from-pinned-checkpoint gives a clean point-in-time restore, and the exactly-once procedure above is sound** for the single-source case (`clone_sources.len()==1`, i.e. restore & rescale base-adoption). Residuals: the post-checkpoint-WAL exclusion is inferred from bounds logic (no targeted test), and rests on `create_checkpoint` snapshotting `next_wal_sst_id` correctly.

Flink's integrated backends hide all of this; here it is your responsibility.
...
- **Disable the embedded worker** to offload: set `compactor_options.worker = null`. This keeps the *coordinator* (scheduler + manifest committer) in-process but stops the *worker* (CPU work); run `slatedb --path <db> run-compactor` as the external worker. ⚠️ omitting the `worker` key defaults it to `Some(...)`, so you must explicitly null it.
...
One-DB-per-subtask means N compactors per TM *and* N shifting compactor targets on every rescale (paths change with clone UUIDs). Shard-per-bucket gives fixed, rescale-invariant DB paths, hence a fixed set of `run-compactor` targets and no re-orchestration.
...
- The **CAS coordination protocol** (`rfcs/0002-compaction.md:184–190, 331–333`): writer and compactor each update the manifest by optimistic **compare-and-swap keyed on `writer_epoch`/`compactor_epoch`**; a CAS conflict causes a **retry, never a wait**, with no shared lock.
...
- *We did not add our own test here:* a single-machine test can't cleanly separate "compaction blocking" from **L0 backpressure** (below), since an over-aggressive flush rate induces backpressure stalls that look like blocking but aren't. SlateDB's in-tree tests already prove the property against the real coordination code.
...
Separately, `await_durable=true` (default) blocks each write until durable in object storage (a WAL flush to S3, ms-scale, group-committed per `flush_interval`=100ms, RFC 0008). **Correction (§18.7):** you *can* run `await_durable=false` in an exactly-once design. The true durability point for the barrier is the §16.2 memtable-flush at `snapshotState` (which makes all pre-barrier writes S3-durable before the Flink checkpoint acks); writes lost on a crash before that are re-generated by Flink source replay. `false` + barrier-flush is exactly-once *and* much faster. `merge` + write batching mitigate further. See also **§18.8: WAL-on-fast-store** (`withWalObjectStore` → S3 Express One Zone) as a first-class latency mitigation, and **§18.9: sustained WAL failure → fatal read-only DB**, not mere backpressure.
...
**The critical interaction:** with `l0_max_ssts_per_key=8` (low, tuned for a steady-state service) and `poll_interval=5s`, bursty stream ingest can stall writes for up to 5 s. **`l0_max_ssts_per_key` is the single most important knob** to keep compaction from stalling your Flink pipeline.
...
**Post-rescale spike:** after a clone/union merge, `l0_max_ssts` can be temporarily exceeded (merged sources' L0 SSTs), so expect transient backpressure right after a downscale until compaction drains. Consider temporarily raising the cap or forcing a compaction before resuming processing.
...
The "block cache" above (`DbCache`) has two interchangeable implementations, both compiled into the published JAR. Both cache the same thing (decoded SST blocks, index, and filters, keyed by SST id) and implement the same `DbCache` trait, so which one you pick is an operational call. One caution: this in-memory block cache is a different layer from the object-store disk cache (§9A), and §9.1b spells out the distinction.
...
- **Moka** is a port of Caffeine, W-TinyLFU eviction, `time_to_live`/`time_to_idle` knobs. Prefer it when you want the idle/TTL eviction behavior. This is what `FlinkShardPerBucketParallelE2E` used (shared moka cache per subtask, §16.12).
- **Foyer** is the default; also memory-only *as reachable from Java*. Fine for the common case (one bounded in-memory block cache per subtask).
...
**You cannot put foyer's cache on disk from Java.** To get SlateDB caching on local SSD, enable the **object-store disk cache** via `root_folder` (§9A, §16.21). The two caches are complementary layers: the object-store cache saves the S3 fetch; the foyer block cache saves the re-parse.
...
```
Point-gets (single block) take a dedup-aware `fetch_block` fast path where concurrent misses for the same block collapse onto one loader (`tablestore.rs:846`). Multi-block reads coalesce uncached blocks into one object-store GET per contiguous run, then insert them (`tablestore.rs:952`). Filters and index blocks follow the same read-through pattern.
...
- ❌ **Not Managed Memory.** RocksDB (integrated) is sized *out of* Managed Memory. **SlateDB has no such integration, so Flink will not carve its footprint out of any section.**
- ✅ Comes out of raw process native memory, at best loosely under JVM Overhead (default ~10%, far too small).

**Consequence:** `container RSS = Flink's full budget + SlateDB's native footprint`, so the cgroup/kernel **OOM-kills the TaskManager** with **no `OutOfMemoryError`, no heap dump, no JVM metric anomaly**, just `exit 137`. This is the hardest OOM class to diagnose and is the *default* outcome without manual reservation.
...
The mirror image of §9 (memory): SlateDB uses **almost no local disk by default**, because its primary data lives in object storage. Three disk paths exist; only one is significant, and it is opt-in.
...
- **By default: no.** SlateDB writes no meaningful local disk, so no contention with RocksDB state, Flink `io.tmp.dirs`, locally-staged checkpoints, logs, or shuffle/spill.
- **If you enable the disk cache: yes, and significantly.** Three cautions:
  ...
  2. **Contends with what Flink needs disk for**, especially in the hybrid design (§13) where RocksDB *is* the hot tier: RocksDB SSTs, local checkpoint dir, `io.tmp.dirs`. A shared volume means contention for **space and IOPS**; a full disk can crash the TM or fail RocksDB flushes.
...
- **Hybrid design (§13):** the sharpest contention point, since hot-tier RocksDB + SlateDB disk cache on one disk will fight. Separate volumes, or skip the disk cache.
- **Standalone compactor (§7):** runs on its own host, so its disk cache is isolated from the TM, another point for the sidecar model.
...
|---|---|---|
| **Object store MUST support conditional writes (VERIFIED, RFC 0001 + txn-obj source):** manifest CAS + single-writer fencing rely on `put_if_not_exists`/`If-Match`; S3 only gained this Aug-2024, some S3-compatible stores lack it | **High (adoption gate)** | verify the target store supports conditional PUT (or use the DynamoDB/MySQL txn-store fallback); **never verified in PoC, only `file://`/`memory://` tested** (§18.1) |
| **Detached-checkpoint lifecycle unmanaged (RFC 0004/0005):** SlateDB cp expiring before the Flink checkpoint that references it → unrestorable state; no-expiry → unbounded SST/manifest leak | **High** | `lifetime=None` (or `refreshCheckpoint`) so the SlateDB cp outlives the retained Flink checkpoint; `deleteCheckpoint` the prior one; retire old clone generations (§18.2) |
...
| **[Hybrid]** Cold-miss read blocks task thread → checkpoint timeout | **High** | ensure cold tail is rarely read; else async/mailbox path (§13.6), which signals wrong tier boundary |
...
11. **⚠️ Object-store conditional-write support (the #1 thing to verify, §18.1).** Manifest CAS + single-writer fencing require `put_if_not_exists`/`If-Match` (VERIFIED in the txn-obj source). Confirm your **exact** target store/endpoint supports conditional PUT: AWS S3 only since Aug-2024; some S3-compatible stores don't. Never exercised in the PoC (`file://`/`memory://` only). If unsupported, you need the DynamoDB/MySQL txn-store fallback (RFC 0001); confirm the 0.14.1 binding even exposes that path.
12. **Detached-checkpoint lifetime vs. Flink retention (§18.2):** confirm `createDetachedCheckpoint` lifetime/refresh/delete semantics from Java and design the protocol so the SlateDB cp outlives the retained Flink checkpoint but doesn't leak. Untested.

7. ~~**Shared-cache key namespacing**~~ ✅ **RESOLVED (safe to share).** Verified in `slatedb/src/db_cache/mod.rs`: `CachedKey { scope_id: u64, sst_id, block_id }`. Each `Db` opened against a shared `DbCache` gets its own `DbCacheWrapper` with a unique `scope_id` (atomic counter from 1), applied to every key via `with_scope(...)`. Source comment: *"ensures that multiple Db instances sharing the same underlying cache do not collide."* Test `test_cache_wrapper_scopes_keys` confirms. **So sharing one `DbCache` across all N/P shards is correctness-safe**, and the memory-multiplier mitigation (§4.1) holds. (Only the bare `CachedKey::from((sst_id, block_id))` path uses reserved `scope_id: 0`; the normal `Db`→`DbCacheWrapper` path, the only one reachable via the Java binding, always scopes. Nothing to do beyond "build one cache, pass to every shard's builder.")
...
Every write goes to both: RocksDB with 1-day State TTL, SlateDB permanently. Reads check RocksDB, and on a miss fall through to SlateDB.
...
1. **Write-cold-before-clear-hot.** Never `hot.clear()` until the SlateDB write is durable. Clearing first then crashing means the key is gone from checkpointed RocksDB *and* the SlateDB write rolled back, i.e. **data loss**. Correct ordering makes the worst case a *duplicate*, never a loss.
...
**Why the ordering is sufficient, verified.** Flink guarantees (a) *"synchronizes invocations of `onTimer()` and `processElement()`"*, so no concurrent state modification, and (b) checkpoints snapshot at a **clean record boundary**: *"all updates to the state from records before the barriers have been made, and no updates that depend on records from after the barriers"*, and barriers *"never overtake records."* So `onTimer` runs to completion between records; a barrier cannot land *between* `cold.put()` and `hot.clear()`. At any barrier, per key, you are either fully before the demotion (hot has it, cold doesn't) or fully after (cold has it, hot cleared), never torn.

> **Subtlety:** Flink's boundary guarantee covers `hot.clear()` (Flink state) but **not** `cold.put()` (an external S3 side effect Flink doesn't track). The guarantee that saves you is the *ordering within the uninterrupted `onTimer`*, not Flink treating both writes atomically. On **recovery**, `cold.put()` durability is independent of the Flink checkpoint, so you may get a duplicate (hot restored + a cold copy). "Hot wins" (rule 2) absorbs it. Same exactly-once trap as §6.6, now spanning two stores: you get **at-least-once with hot-wins dedup**, not free exactly-once.
...
- **The alignment that makes it correct:** both tiers must partition by the **same key groups with the same `maxParallelism`**. Flink redistributes hot state by key group; if SlateDB's key-group prefix uses that identical `maxParallelism`, then after rescale a subtask owns the **same key set in both tiers**, so no key can end up hot-here / cold-there. **This is the single most important cross-tier constraint.** Shard-per-bucket with buckets = key-group ranges aligns naturally with Flink's redistribution *and* gives zero-copy cold rescale (§4).
...
> - **`AbstractStreamOperator<Out>`** (classic, V1) implements `YieldingOperator`, which declares the **setter** `setMailboxExecutor(MailboxExecutor)` (`@Internal`). No getter, so capture the reference into a field when it's set. `MailboxExecutor` is in `org.apache.flink.api.common.operators`.
...
**The catch:** you must **drain all in-flight `cold.get` continuations before the snapshot completes**. Override `prepareSnapshotPreBarrier`/`snapshotState` in the operator and block until pending futures resolve (else a checkpoint captures hot state missing a pending update → inconsistency). Also **bound in-flight requests** (backpressure when too many cold reads outstanding) and handle ordering. Building this correctly means **re-implementing Flink 2.0's async state framework by hand**, which 1.20 does not provide.
...
- **Cost:** many small physical DBs bring the N/P per-subtask overhead of §4.1 (compactors, unflushed buffers). Mitigated by shared cache (§12.7, safe), sidecar compaction (§7), trimmed `max_unflushed_bytes`. This is the price of never physically resharding; pay it once by design.
...
- ❌ **No native DB split or DB merge**; both are "under development" in README. You cannot split one DB into two by key range, or merge two DBs, as a primitive.
...
3. **Split** a shard: two clones from the same checkpoint, each with a `CloneSourceSpec` carrying `withCheckpoint(path, uuid)` **and** a `projection_range` `KeyRange` for one half's key-group byte-range, producing two new DBs.
   **Merge** shards: `scan` each source's key-group range, then `put` into one new DB.
...
- Shallow clone references full parent SSTs (projection is not a physical prune): clones design doc.
...
2. **Compaction-filter exposure is nuanced, and TTL is weaker than it looks (corrected, §18.6, VERIFIED by `SlateDbTtlE2E`).** `putWithOptions(PutOptions{ttl})` / `mergeWithOptions` **ARE bound** (verified `javap`), so TTL is *settable* from Java. **But, established by running, SlateDB TTL does NOT filter on read**: a point-`get` after expiry still returns the value; the value is only dropped by a later compaction *merge* (lazy space reclamation). So it is **not** a substitute for Flink State TTL's read-time expiry; a Flink cold tier must still check expiry itself. TTL is also **wall-clock** (processing-time `OnCreateAndWrite` only, not event-time/`OnReadAndWrite`; `clock` not injectable). Separately, *arbitrary custom* compaction filters (RFC 0017) are unbound **and compiled off** in the shipped JAR. So the earlier "TTL gap is closed" framing was itself wrong (see §18.6).
3. *(nice-to-have, non-blocking)* column families (prefix multiplexing works, §6.1) and SST-ingest (clone + scan-copy works, §6.4).

**Meta-answer:** the bulk of "what's missing" is the **Flink integration layer**: `KeyedStateBackend` SPI, checkpoint-coordinator wiring, key-group rescale, timer service, and async execution. Building all that is "reimplement ForSt with a SlateDB core" (§2). This is why, on **2.3**, the pragmatic path is **ForSt**; and why on **1.20** (no async state at all) a true drop-in RocksDB replacement is not achievable. The hybrid tiering of §13 (RocksDB hot + SlateDB cold via a ProcessFunction) is the realistic way to get SlateDB into the picture without the full backend.
...
**Impact if missed:** every Flink checkpoint would pin an empty/stale SlateDB manifest, i.e. silent state loss on restore.
...
- source rewound to checkpointed **offset 20**; operator cloned SlateDB from the checkpoint id captured in
  the *same* Flink checkpoint (`db-gen-0 @cp=…` → `db-gen-1`), so offset and SlateDB-cp restore to the **same
...
(2) `env.fromData` emits all records instantly so no checkpoint captures mid-stream state, so a **throttled,
...
- **§12.7 shared cache across instances**: scope-id collision-safety confirmed LIVE under concurrent load
  (not just source-read), with exact values across 4 shards sharing one moka cache per subtask.
...
write-first then `hot.clear()`), **late re-touch promotes from SlateDB** (`hot=3` = cold 2 + 1), then re-demote.
...
3. **The flat ~1 ms/batch PUT is DURABILITY, not marshalling.** Verified in source: `WriteOptions.await_durable = true` by default, so every `write()`/`put()` blocks until the WAL flush. Flat ~1 ms regardless of size/batch confirms it. Batching 100 rows amortizes that one wait to ~10 µs/row effective.

**Context that makes this a non-issue for the SlateDB-in-Flink design:** a cache-**miss** get is an S3 GET at ~1–10 ms = **100–200× the entire FFI+copy+async cost**. Storage latency dwarfs marshalling. **Practical guidance:** don't do chatty one-at-a-time `await` per key (caps ~20k ops/s/thread); **pipeline or use `WriteBatch`** to reach ~70–97k/s and to amortize the `await_durable` WAL flush. Value size barely matters for marshalling below the multi-MB range. *Scope caveat:* single machine, `memory:///`, single-thread driver, so absolute µs will shift under real Flink concurrency + S3, but the ratios (storage ≫ latency ≫ copy cost) are the robust conclusion. The JNA binding (§17) has somewhat higher per-call *latency* (reflective dispatch vs `MethodHandle`), same copy cost.

### 16.15 READ-YOUR-WRITES before any flush (`ReadYourWritesE2E`)

Verifies the pre-flush consistency guarantee §6.2 relies on: within a single `Db` instance, a read sees your own writes **before any `flush`/`checkpoint`**. Ran on `memory:///` (no disk/S3 that could mask unflushed data) with **no flush anywhere**, so the only way a read succeeds is via the in-memory memtable path. All 8 checks passed:
...
- **SPLIT via projection**: cloned the source twice with `CloneBuilder.withProjectionRange`: `shard-lo`=[key-000000, key-000300), `shard-hi`=[key-000300, key-000599]. ✅ Each shard has **exactly its half** (300 keys, 0 wrong, nothing leaked across the boundary, since projection physically clips).
...
This graduates the §6.4 rescale from "algorithm verified standalone + inferred-in-Flink" to **verified end-to-end inside a real Flink savepoint/rescale cycle**. Scope/caveats: single-JVM MiniCluster on `file:///` (not real S3, not multi-node); the union runs inside `initializeState` synchronously (fine at this scale, but a large downscale union would extend restore time, and the §18.2 checkpoint-lifetime and cold-start-warming concerns still apply at scale). Union preconditions held automatically because key-group ranges are contiguous, non-overlapping, and adjacent by construction (§6.1 big-endian prefix, §18.3).
...
generator* and touch neither Rust nor the native lib:
...
## 18. RFC-Sweep Gaps & Corrections

This doc grew from a question-driven investigation, so whole capability areas were never prompted. A systematic read of **all 28 SlateDB RFCs** (`slatedb/rfcs/`) against this doc surfaced the gaps below. Each is tagged **[VERIFIED]** (I confirmed it against v0.14.1 source / the published JAR via `javap`) or **[RFC]** (stated by the RFC; verify before relying), ordered by importance.

### 18.1 ⚠️ Object store MUST support conditional writes (RFC 0001) — **[VERIFIED]**, adoption gate
SlateDB's manifest CAS, the substrate for single-writer fencing (§4) **and** the §6.6 exactly-once story, relies on object-store **conditional PUT**. The transactional-object source is explicit: *"We rely on `put_if_not_exists` to enforce CAS"* (`PutMode::Create` + `PutMode::Update(version)` / `If-Match`). RFC 0001: *"Most object stores provide CAS… **But S3 does not**, and we want to support S3,"* proposing a **DynamoDB/MySQL** external-txn fallback for stores without it.
- **Consequence:** the object-store choice is load-bearing for *correctness*, not just performance. **AWS S3 only added conditional PUT (`If-None-Match`) in Aug 2024**; older/on-prem MinIO, various S3 gateways, or a misconfigured bucket may lack it, leaving the fencing/exactly-once mechanism silently without a substrate.
- **Blind spot:** every PoC ran on `file:///`/`memory:///`, which provide atomic/CAS semantics for free, so the single most safety-critical external dependency was **never exercised**. Before adopting: confirm your target store supports conditional writes (or wire the txn-store fallback, if the binding exposes it, which is unverified). Amends §2.1 (store list), §4, §6.6.

### 18.2 Detached-checkpoint lifecycle is unmanaged (RFC 0004/0005) — exactly-once hole
§6.3 calls `createDetachedCheckpoint` on **every** Flink checkpoint and stores the id, but never manages the checkpoint's *lifetime* against Flink's retention:
- **Expiry → unrestorable.** A SlateDB checkpoint has `checkpoint_expire_time_s`; if it expires (or is GC'd) before Flink restores from a *retained* checkpoint/savepoint (Flink may hold these for days), the clone source is gone, i.e. **silent state loss on restore**. Use `lifetime=None` or `refreshCheckpoint` to guarantee it outlives the referencing Flink checkpoint.
- **No-expiry → unbounded leak.** With `lifetime=None`, checkpoints accumulate: each pins its manifest version + superseded SSTs, so the manifest checkpoint-list and object-store footprint grow every checkpoint interval **forever** unless you `deleteCheckpoint` the prior one. The protocol must delete cp[N-k] keyed to Flink's retained-checkpoint count.
- **No generation retirement.** Clone-on-restore (§6.6) and rescale (§6.4) mint a new-path DB each time; the doc's only reclamation story (§6.5/§16.13) is orphan-SST GC *within one live DB*. Old whole-DB generations need `Db::destroy` + a per-DB GC task (RFC 0004), otherwise they leak. **[RFC]**
- **Iterator invalidation (RFC 0005):** a long downscale scan-copy (§6.4) over a checkpoint-pinned source throws `InvalidatedIterator` if that checkpoint expires mid-scan → **partial migration → silent loss**. The scan-copy loop needs invalidation/retry handling. Also: scans have **no snapshot isolation** (torn reads), so the rescale copy MUST read a checkpoint-pinned `DbReader`, not a live `Db`. **[RFC]**

### 18.3 Big-endian key-group prefix is mandatory (RFC 0016 REJECTED) — **[VERIFIED]** correctness
Custom comparators were **rejected**, so SlateDB is bytewise-lexicographic **permanently, with no comparator escape hatch**. So §6.1's key-group prefix (1–2 bytes; §14 uses maxParallelism up to 32768 → 2 bytes) **must be encoded big-endian**, or "a range of key groups = one contiguous byte range" silently breaks (little-endian would sort key group 256 before 1), and the *entire* rescale scan-copy (§6.4), clone-projection `KeyRange` (§6.6/§16.17), and prefix bloom filters (§5B) depend on that property. (Flink's own key-group serialization is big-endian for the same reason.) Likewise §6.1a's "byte-order-preserving value encoding" is a **hard, permanent constraint** for any ordered MapState/range scan, not a caveat. Amends §6.1.
...
- `slatedb.db.total_mem_size_bytes` + `wal_buffer_estimated_bytes` → **the §9 silent-OOM gauges**
- `slatedb.db_cache.access_count{result=hit|miss}` → **cache hit rate (§9)**; `slatedb.object_store_cache.*` → disk-cache (§9A)
- `slatedb.db.request_duration_seconds{op}` histogram → **read/write latency incl. S3 p99/p999 (§11)**

⚠️ **Gotcha:** the default recorder is **no-op, so zero metrics unless you wire one** (`withMetricsRecorder`). And whether a Java operator reads snapshots directly vs. implements the `MetricsRecorder` callback interface (FFI hot-path cost) is a real binding-usability question (RFC 0021 flags UniFFI exposure as follow-up). New §-worthy area; amends §9.4.

### 18.6 TTL from Java: lazy compaction-reclaim, NOT read-time expiry (RFC 0003/0017) — **[VERIFIED by running, `SlateDbTtlE2E`]** — corrects §15.3 *and* an earlier wrong "correction"
`putWithOptions(PutOptions{ttl})` / `mergeWithOptions` **ARE bound** (verified `javap`), and SlateDB has a built-in TTL retention filter, so TTL *is* usable from Java. **But testing overturned the assumption that expiry is applied on read.** Verified behavior in 0.14.1 (`SlateDbTtlE2E`, `ExpireAfterTicks(2000)`):
- `expire_ts` is set correctly (a real future-ms timestamp on the `KeyValue`).
- **A point-`get` after the TTL elapses still RETURNS the value**; it is **not** filtered on read. `getKeyValue` returns it with `expireTs` populated. (The get-path `DbIterator` carries no clock; expiry is not evaluated there.)
- The value is dropped **only after a real compaction *merge*** rewrites the SST (the `RetentionIterator` runs with a clock during compaction). Confirmed: still present after a plain flush; NULL only once enough L0 SSTs forced a merge.

⇒ **SlateDB TTL is lazy, compaction-driven *space reclamation*, not a read-time "returns null when expired" guarantee.** This is materially weaker than Flink State TTL (which filters on read) and than the built-in TTL filter Flink relies on in RocksDB. **For Flink cold-tier state you MUST still filter expiry yourself** (store an expiry field and check it in the operator, or use explicit demotion timers §13.7); you cannot rely on `get` hiding expired state; TTL only bounds *storage growth* eventually. Other limits confirmed: TTL is **wall-clock** (processing-time `OnCreateAndWrite` only, not event-time / `OnReadAndWrite` refresh; the `clock` is a builder Component **not injectable** in the binding, no `with_clock`). And *arbitrary custom* compaction filters (RFC 0017) remain unbound + **compiled off** in the JAR, with a snapshot-consistency hazard for §6.6.

### 18.7 `await_durable=true` is overstated as mandatory (RFC 0008) — corrects §7.3
The barrier memtable-flush (§16.2) is the true durability point for exactly-once, so `await_durable=false` + barrier-flush is still exactly-once and much faster (crash-lost uncheckpointed writes are replayed by the Flink source). Also missing from §7.3/§9.3: `await_durable` writes **group-commit per `flush_interval`** (100ms default), which is the real per-write wait knob; and a **sustained WAL-write failure / `max_unflushed_bytes` exhaustion puts the DB into a fatal read-only state** (not mere backpressure), so a Flink operator would fail under a prolonged S3 outage.

### 18.8 WAL-on-fast-store latency mitigation (RFC 0009) — **[VERIFIED]** `withWalObjectStore` bound
The doc flags ~1ms/write WAL-flush latency (§7.3/§16.14) as the #1 write cost but never proposes RFC 0009's direct fix: put the WAL on a **low-latency object store (S3 Express One Zone)** via `withWalObjectStore` (verified bound). Trade-off: One Zone is single-AZ (lower WAL durability), tolerable since Flink replays uncheckpointed writes, but a real choice. Amends §7.3/§8.

### 18.9 Operator error contract (RFC 0007) — **[VERIFIED]** `CloseReason.Fenced` exists
The doc covers the fencing *mechanism* but not the *error a Flink operator catches*. Verified in `error.rs`: a fenced subtask surfaces as `Error.Closed{reason=Fenced}` on the next call, so the operator MUST treat that `Db` handle as dead (fail the task → Flink restarts + clone-restores; a fresh instance is required). And triage matters: `Unavailable`/`Transaction` are **retryable** (S3/network blips that should NOT fail the job), while `Closed`/`Data`/`Internal` are **fatal**. The blocking-RMW path (§6.2) has no error-triage story today; `Error` is `#[non_exhaustive]` (needs a default-fatal branch). Amends §6.2/§11.

### 18.10 Smaller items (verify before relying)
- **CDC/`WalReader` is shipped in the Java binding (RFC 0019)** [VERIFIED exports]: offers an *alternative* exactly-once path ("write output + WAL position atomically") beyond clone-restore; the doc mentions it nowhere. Its WAL-vs-memtable-flush ordering note is load-bearing background for §16.2/§6.6.
- **Cache warming after clone (RFC 0023)** [RFC]: `warm_sst`/`evict_cached_sst` (on `Db` *and* `DbReader`) would kill cold-read latency right after clone-restore (§6.6) / rescale (§6.4), the exact §13.6 "cold read blocks task thread → checkpoint stall" case. Likely not in 0.14.1 binding; verify.
- **Prefix filters don't help range scans (RFC 0022)** [RFC]: `withFilterPolicies` prefix blooms accelerate only `scan_prefix`, **not** plain range `scan` (used by §6.4 rescale-copy and MapState iteration). And a **sidecar compactor must be configured with the same `withFilterPolicies` AND the same merge operator (RFC 0006), or filters/folds are silently wrong**; amends §7.
- **Custom merge operator from Java unverified (RFC 0006)** [RFC]: §6.2 leans on `db.merge` for race-free RMW, but whether a *Java-defined* merge function can be supplied (vs. Rust-only) is not established; if it can't, the sidecar-compaction recommendation (§7) is incompatible with merge-based RMW (compactor must run the same operator).
- **Transactions (RFC 0011)** [RFC]: §5.2 tags `begin`/SSI as "verified shipped"; the PoC never exercised a transaction. `WriteBatch` (confirmed) already gives single-writer multi-key atomicity; verify SSI actually ships in 0.14.1 before designing around it. Long-running txns/snapshots auto-expire (another native-memory consumer §9 omits).
- **Subcompactions (RFC 0028)** [RFC, likely post-0.14.1]: a forthcoming `max_subcompactions` (default 4) multiplies intra-compaction parallelism (`max_concurrent_compactions × max_subcompactions` ≈ 16 CPU threads/DB), so tune down for embedded use; another argument for the sidecar (§7).
- **Per-subtask size via `manifest()`/`estimate_size()` (RFC 0020)** [RFC]: a no-I/O way to measure *actual* per-subtask DB size (§9A/§5D) and pre-estimate downscale scan-copy cardinality, instead of guessing from config; over-counts 4–8× while data sits in L0. Likely Rust-side, verify binding.

### 18.11 What remains adequately covered (not gaps)
Compaction non-blocking + backpressure (§7/§8), checkpoints/clone/projection/union (§6.4/§6.6/§16.17), on-disk formats (§5C), the FFM/JDK-22 floor and JNA workaround (§17), memory model (§9), marshalling cost (§16.14), read-your-writes (§16.15). WAL-format RFC 0014 is **rejected** and the doc correctly reflects the post-rejection reality (WAL = SST format).

### 18.12 Two sweep findings graduated from reading to RUNNING (one confirmed, one corrected)
...
(Full 43-name dump is in `SlateDbMetricsE2E` output. Note `object_store.request_duration_seconds` is the **histogram**, where the S3 tail-latency the doc worried about actually surfaces; the earlier RFC-guessed `db.request_duration_seconds` name was wrong.)
